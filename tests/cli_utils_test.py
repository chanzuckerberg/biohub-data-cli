...
from biohub_data_cli.utils.cli import (
    DownloadDisplay,
    safe_join,
)
...
# ── DownloadDisplay.add_dataset_download_task / get_total_bytes_downloaded ──


def test_get_total_bytes_downloaded_sums_across_tasks():
    d = DownloadDisplay()
    t1 = d.add_dataset_download_task("coll-a/ds-1", total=1000)
    t2 = d.add_dataset_download_task("coll-a/ds-2", total=1000)
    t3 = d.add_dataset_download_task("coll-b/ds-1", total=1000)
    d.advance_task(t1, 100)
    d.advance_task(t2, 250)
    d.advance_task(t3, 50)
    assert d.get_total_bytes_downloaded() == 400
