...
from data_cli.utils.http import download_http, http_url_to_local_path
...
def test_download_http_success(tmp_path):
    ...
    with patch("data_cli.utils.http.requests.get", return_value=mock_response):
...
def test_download_http_records_failure(tmp_path):
    with patch("data_cli.utils.http.requests.get", side_effect=OSError("timeout")):
        result = download_http(
            "https://example.com/file.h5ad",
            tmp_path,
            "my-coll",
